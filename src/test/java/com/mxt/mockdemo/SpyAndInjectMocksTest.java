...
public class SpyAndInjectMocksTest {


    /**
     * @InjectMocks 只能配合 @Mock/@Spy 使用，不能使用@SpyBean,会报NPE。
     * @InjectMocks 不能注解到接口上
     * 使用@InjectMocks,即使用了@Spy，如果其更下一次没有mock（dao层），其也会返回null。
     *  @Mock/@Spy的区别还是在于，spy会执行被@Spy注解的类的真实方法（前提是在该层就有确切的返回）
     */
